  ...
  <div>
    ...
    <form method="post" @submit.prevent="onFormSubmit">
      <div class="card mx-auto registerFormContainer">
        <div class="card-title text-center formTitle my-3 mb-0">
          Inscription
        </div>
        <div
          class="card-body d-flex flex-column mx-auto p-0 justify-content-around w-50"
        >
          <div>
            ...
            <input
              ...
              v-model="name"
              name="name"
              class="form-control formInput mx-auto"
              required
            />
            <div v-if="errors['name']" class="mt-1 text-danger formError">
              {{ errors["name"] }}
            </div>
          </div>
          <div>
            ...
            <input
              ...
              required
            />
            <div v-if="errors['firstName']" class="mt-1 text-danger formError">
              {{ errors["firstName"] }}
            </div>
          </div>
          <div>
            ...
            <input
              ...
              required
            />
            <div v-if="errors['login']" class="mt-1 text-danger formError">
              {{ errors["login"] }}
            </div>
          </div>
          <div>
            ...
            <input
              ...
              required
            />
            <div v-if="errors['password']" class="mt-1 text-danger formError">
              {{ errors["password"] }}
            </div>
          </div>
          <div class="mb-3">
            ...
            <div
              ...
              class="mt-1 text-danger formError"
            >
              {{ errors["confirmPassword"] }}
            </div>
          </div>
        </div>
        <button class="btn w-50 mx-auto formSubmit mt-3" type="submit">
          S'inscrire
        </button>
        <router-link
          :to="{ name: 'login' }"
          class="mx-auto mt-2 small text-dark mb-4 formSubLabel"
  ...
  methods: {
    onFormSubmit: function (e) {
      ...
      if (this.login.length < 3) {
        this.errors["login"] =
          "Nom d'utilisateur trop court (3 caractères min)";
      }
